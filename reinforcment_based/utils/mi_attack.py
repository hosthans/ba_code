...
def inversion_dcgan(agent, G, T, alpha, z_dim = 100, max_episodes=40000, max_step=1, label=0, model_name="VGG16"):
    print("Target Label : " + str(label))
    best_score = 0
    c = None
...
def inversion_MNIST_DP(agent, G, T, alpha, z_dim = 100, max_episodes=40000, max_step=1, label=0, model_name="VGG16"):
    print("Target Label : " + str(label))
    best_score = 0
    c = None
    ...
    for i_episode in range(1, max_episodes + 1):
        ...
        if mean_score >= best_score:
            best_score = mean_score
            best_images = torch.vstack(test_images)
            os.makedirs("./attack_results/reinforcment_mnist_dp/images/{}".format(model_name), exist_ok=True)
            os.makedirs("./attack_results/reinforcment_mnist_dp/models/{}".format(model_name), exist_ok=True)
            save_image(best_images, "./attack_results/reinforcment_mnist_dp/images/{}/{}.png".format(model_name, label), nrow=10)
            torch.save(agent.actor_local.state_dict(), "./attack_results/reinforcment_mnist_dp/models/{}/actor_{}_{}.pt".format(model_name, label, alpha))
        if i_episode % 10000 == 0 or i_episode == max_episodes:
            print('Episodes {}/{}, Confidence score for the target model : {:.4f}'.format(i_episode, max_episodes,best_score))
            save_image(best_images, "./attack_results/reinforcment_mnist_dp/images/{}/final_image_{}_{}.png".format(model_name, i_episode, label), nrow=10)

    return best_images


def inversion_MNIST_I(agent, G, T, alpha, z_dim = 100, max_episodes=40000, max_step=1, label=0, model_name="VGG16"):
    print("Target Label : " + str(label))
    best_score = 0
    c = None
